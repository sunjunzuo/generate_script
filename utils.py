...
def generate_script(subject, video_length,
    ...
    title_template = ChatPromptTemplate.from_messages(
        [
        ("human", "请为'{subject}'这个主题的视屏想一个吸引人的标题")
                                                      ]
    )
    script_template = ChatPromptTemplate.from_messages(
        [
            ("human",
             ...
             视屏标题：{title}，视频时长：{duration}分钟，生成的脚本长度尽量遵循视频时长的要求。
             要求开头抓住眼球，中间提供干货内容，结尾有惊喜，脚本格式请按照【开头，中间，结尾】分割。
...
print(generate_script("大型语言模型",1, 0.7, os.getenv("OPENAI_API_KEY")))